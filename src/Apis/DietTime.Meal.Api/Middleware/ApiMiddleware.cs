...
public sealed class ExceptionMiddleware(RequestDelegate next, ILogger<ExceptionMiddleware> logger, IHostEnvironment environment)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (Exception ex)
        {
            ...
            var (status, title) = ex switch { ValidationException => (400, "Validation failed"), UnsupportedLanguageException => (400, "Unsupported language"), InvalidGuestSessionException => (401, "Invalid guest session"), ArgumentException => (400, "Invalid request"), DbUpdateConcurrencyException => (409, "Concurrency conflict"), DbUpdateException => (409, "Database conflict"), InvalidOperationException => (422, "Request cannot be processed"), _ => (500, "An unexpected error occurred") };
