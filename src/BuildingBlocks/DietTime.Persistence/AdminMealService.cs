...
public sealed class AdminMealService(DietTimeDbContext db, TimeProvider clock) : IAdminMealService
{
    ...
    {
        var x = await db.MealItems.AsNoTracking().AsSplitQuery().Include(m => m.Translations).Include(m => m.Nutrition).Include(m => m.Ingredients).Include(m => m.Allergens).Include(m => m.Tags).Include(m => m.Variants).ThenInclude(v => v.Translations).Include(m => m.Prices).SingleOrDefaultAsync(m => m.Id == mealId, ct);
        ...
        var variants = x.Variants.OrderBy(v => v.DisplayOrder).Select(v => new AdminVariantRequest(v.Code, v.AdditionalPrice, v.CaloriesDifference, v.ProteinDifferenceGrams, v.CarbohydratesDifferenceGrams, v.FatDifferenceGrams, v.IsDefault, v.IsActive, v.DisplayOrder, v.Translations.Select(t => new AdminTranslationRequest(t.LanguageCode, t.Name, t.Description, null)).ToArray())).ToArray();
        ...
        return new(x.Id, x.Status, new(x.Sku, x.CategoryId, x.PreparationTimeMinutes, x.IsVegetarian, x.IsVegan, x.IsGlutenFree, x.IsDairyFree, x.IsAvailable, x.AvailableFrom, x.AvailableUntil, translations, nutrition, ingredients, allergens, x.Tags.Select(t => t.TagId).ToArray(), variants, prices, x.Status, x.IsSpicy, x.SpiceLevel));
    }
    ...
    public async Task<bool> UpdateMealAsync(Guid mealId, UpsertMealRequest request, Guid? userId, CancellationToken ct)
    {
        ...
        var meal = await db.MealItems.Include(x => x.Translations).Include(x => x.Nutrition).Include(x => x.Ingredients).Include(x => x.Allergens).Include(x => x.Tags).Include(x => x.Variants).ThenInclude(x => x.Translations).Include(x => x.Prices).SingleOrDefaultAsync(x => x.Id == mealId, ct); 
        if (meal is null) 
            return false; 
        var now = clock.GetUtcNow();
        ...
        db.MealVariants.RemoveRange(meal.Variants); 
        db.MealPrices.RemoveRange(meal.Prices); 
        meal.Translations.Clear(); meal.Ingredients.Clear();
        meal.Allergens.Clear(); meal.Tags.Clear(); meal.Variants.Clear(); meal.Prices.Clear(); ApplyTranslations(meal, request.Translations, now); ApplyNutrition(meal, request.Nutrition, now); ApplyAggregate(meal, request, now, userId); await db.SaveChangesAsync(ct); await tx.CommitAsync(ct); return true;
    }
    ...
    private static void ApplyAggregate(MealItem meal, UpsertMealRequest request, DateTimeOffset now, Guid? userId)
    {
        ...
        foreach (var x in request.Variants ?? []) { var variant = new MealVariant { Code = x.Code, AdditionalPrice = x.AdditionalPrice, CaloriesDifference = x.CaloriesDifference, ProteinDifferenceGrams = x.ProteinDifferenceGrams, CarbohydratesDifferenceGrams = x.CarbohydratesDifferenceGrams, FatDifferenceGrams = x.FatDifferenceGrams, IsDefault = x.IsDefault, IsActive = x.IsActive, DisplayOrder = x.DisplayOrder, CreatedAt = now, UpdatedAt = now, CreatedBy = userId, UpdatedBy = userId }; foreach (var t in x.Translations) variant.Translations.Add(new() { LanguageCode = t.LanguageCode, Name = t.Name, Description = t.ShortDescription, CreatedAt = now, UpdatedAt = now }); meal.Variants.Add(variant); }
