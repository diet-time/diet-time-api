...
public static class DevelopmentSeeder
{
    public static async Task SeedAsync(DietTimeDbContext db, CancellationToken ct = default)
    {
        ...
        var tag = new MealTag { Id = Id(5), Code = "LOW_CARB", IsActive = true, CreatedAt = now, UpdatedAt = now, Translations = [new() { LanguageCode = "en", Name = "Low Carb", CreatedAt = now, UpdatedAt = now }, new() { LanguageCode = "ar", Name = "قليل الكربوهيدرات", CreatedAt = now, UpdatedAt = now }] };
        ...
        meal.Ingredients.Add(new() { Id = Id(9), Ingredient = ingredient, Quantity = 30, Unit = "g", IsPrimaryIngredient = true, CreatedAt = now }); meal.Allergens.Add(new() { Allergen = allergen, AllergenLevel = "MAY_CONTAIN", CreatedAt = now }); meal.Tags.Add(new() { Tag = tag, CreatedAt = now }); meal.Prices.Add(new() { Id = Id(10), Amount = 25, CurrencyCode = "QAR", PriceType = "INDIVIDUAL", EffectiveFrom = now.AddDays(-1), IsActive = true, CreatedAt = now, UpdatedAt = now });
