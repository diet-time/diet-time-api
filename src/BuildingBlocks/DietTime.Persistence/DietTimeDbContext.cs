    ...
    : IdentityDbContext<ApplicationUser, IdentityRole<Guid>, Guid>(options)
{
    ...
    private static void ConfigureMeals(ModelBuilder b)
    {
        ...
        b.Entity<MealMedia>(e => { e.ToTable("meal_media"); e.Property(x => x.MediaType).HasMaxLength(30); e.Property(x => x.ObjectKey).HasMaxLength(1000); e.Property(x => x.PublicUrl).HasMaxLength(2000); e.Property(x => x.ThumbnailUrl).HasMaxLength(2000); e.HasIndex(x => x.EntityId).HasDatabaseName("ix_meal_media_entity"); e.HasIndex(x => new { x.EntityId, x.MediaType }).IsUnique().HasFilter("is_primary = true AND status = 'ACTIVE'").HasDatabaseName("ux_meal_media_primary_entity_type"); });
    ...
    {
        b.Entity<MealPlanTemplate>(e => { e.ToTable("meal_plan_templates"); e.HasIndex(x => new { x.VersionGroupId, x.VersionNumber }).IsUnique().HasDatabaseName("ux_meal_plan_templates_version"); e.HasIndex(x => x.Code).IsUnique().HasFilter("is_latest = true").HasDatabaseName("ux_meal_plan_templates_latest_code"); e.Property(x => x.RowVersion).IsConcurrencyToken(); e.HasIndex(x => new { x.IsPublished, x.IsActive }).HasDatabaseName("ix_meal_plan_templates_published"); });
        ...
        {
            e.ToTable("meal_package_options");
            e.Property(x => x.Name).HasMaxLength(150).IsRequired();
            e.HasIndex(x => x.Name).IsUnique().HasDatabaseName("ux_meal_package_options_name");
            e.ToTable(t =>
            {
                t.HasCheckConstraint("ck_meal_package_options_meal_count", "meal_count > 0");
                t.HasCheckConstraint("ck_meal_package_options_snack_count", "snack_count >= 0");
            });
        });
        b.Entity<MealPackageOptionType>(e =>
        {
            e.ToTable("meal_package_option_types");
            e.HasIndex(x => new { x.PackageOptionId, x.MealTypeId }).IsUnique()
                .HasDatabaseName("ux_meal_package_option_types_package_meal_type");
