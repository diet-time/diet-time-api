    ...
    : IdentityDbContext<ApplicationUser, IdentityRole<Guid>, Guid>(options)
{
    ...
    public DbSet<MealVariant> MealVariants => Set<MealVariant>();
    public DbSet<MealVariantTranslation> MealVariantTranslations => Set<MealVariantTranslation>();
    ...
    public DbSet<MealPlanTemplateDayTranslation> MealPlanTemplateDayTranslations => Set<MealPlanTemplateDayTranslation>();
    ...
    private static void ConfigureMeals(ModelBuilder b)
    {
        ...
        b.Entity<MealVariant>(e => { e.ToTable("meal_variants"); e.Property(x => x.AdditionalPrice).HasPrecision(12, 2); e.Property(x => x.CaloriesDifference).HasPrecision(10, 2); e.Property(x => x.ProteinDifferenceGrams).HasPrecision(10, 2); e.Property(x => x.CarbohydratesDifferenceGrams).HasPrecision(10, 2); e.Property(x => x.FatDifferenceGrams).HasPrecision(10, 2); e.HasIndex(x => new { x.MealItemId, x.Code }).IsUnique(); e.HasOne(x => x.MealItem).WithMany(x => x.Variants).HasForeignKey(x => x.MealItemId).OnDelete(DeleteBehavior.Cascade); });
        Translation<MealVariantTranslation>(b, "meal_variant_translations", 10); b.Entity<MealVariantTranslation>(e => { e.HasIndex(x => new { x.MealVariantId, x.LanguageCode }).IsUnique(); e.HasOne(x => x.Variant).WithMany(x => x.Translations).HasForeignKey(x => x.MealVariantId).OnDelete(DeleteBehavior.Cascade); });
    }
    ...
    private static void ConfigurePlans(ModelBuilder b)
    {
        ...
        b.Entity<MealPlanTemplateDay>(e => { e.ToTable("meal_plan_template_days"); e.HasIndex(x => new { x.MealPlanTemplateId, x.DayNumber }).IsUnique(); e.HasIndex(x => x.MealPlanTemplateId).HasDatabaseName("ix_meal_plan_days_template"); e.HasOne(x => x.Plan).WithMany(x => x.Days).HasForeignKey(x => x.MealPlanTemplateId).OnDelete(DeleteBehavior.Cascade); });
        Translation<MealPlanTemplateDayTranslation>(b, "meal_plan_template_day_translations", 10); b.Entity<MealPlanTemplateDayTranslation>(e => { e.HasIndex(x => new { x.MealPlanTemplateDayId, x.LanguageCode }).IsUnique(); e.HasOne(x => x.Day).WithMany(x => x.Translations).HasForeignKey(x => x.MealPlanTemplateDayId).OnDelete(DeleteBehavior.Cascade); });
