...
public sealed class CatalogueApiTests : IAsyncLifetime
{
    ...
    [Fact] public async Task Guest_home_is_public_and_returns_plans_slots_and_dates_without_meals() { if (!enabled) return; var response = await client!.GetAsync("/api/v1/guest/home?date=2026-07-23"); Assert.Equal(HttpStatusCode.OK, response.StatusCode); var body = await response.Content.ReadAsStringAsync(); using var json = System.Text.Json.JsonDocument.Parse(body); var data = json.RootElement.GetProperty("data"); Assert.Equal(2, data.GetProperty("mealPlans").GetArrayLength()); var selectedPlan = data.GetProperty("mealPlans").EnumerateArray().Single(x => x.GetProperty("isSelected").GetBoolean()); Assert.Equal("CLASSIC", selectedPlan.GetProperty("code").GetString()); Assert.Equal("Classic short description", selectedPlan.GetProperty("description").GetString()); Assert.Equal("https://cdn.test/plan.png", selectedPlan.GetProperty("imageUrl").GetString()); var slot = Assert.Single(selectedPlan.GetProperty("slots").EnumerateArray()); Assert.Equal("BREAKFAST", slot.GetProperty("mealTime").GetProperty("code").GetString()); Assert.False(slot.TryGetProperty("meals", out _)); Assert.Equal(7, data.GetProperty("weeklyCalendar").GetArrayLength()); Assert.False(data.TryGetProperty("menus", out _)); Assert.False(data.TryGetProperty("pagination", out _)); }
