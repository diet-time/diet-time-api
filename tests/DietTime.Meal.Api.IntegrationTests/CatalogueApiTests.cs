...
public sealed class CatalogueApiTests : IAsyncLifetime
{
    ...
    [Fact] public async Task Guest_home_ignores_meal_time_and_paging_filters_in_the_default_response() { if (!enabled) return; var body = await client!.GetStringAsync("/api/v1/guest/home?date=2026-07-23&mealTimeCode=BREAKFAST&page=1&pageSize=1"); using var json = System.Text.Json.JsonDocument.Parse(body); var data = json.RootElement.GetProperty("data"); var selectedPlan = data.GetProperty("mealPlans").EnumerateArray().Single(); Assert.Single(selectedPlan.GetProperty("slots").EnumerateArray()); Assert.False(data.TryGetProperty("mealTimeFilters", out _)); Assert.False(data.TryGetProperty("pagination", out _)); }
    [Fact] public async Task Guest_home_returns_only_the_selected_plan_and_selected_date_menu_by_default() { if (!enabled) return; var body = await client!.GetStringAsync("/api/v1/guest/home?date=2026-07-23"); using var json = System.Text.Json.JsonDocument.Parse(body); var data = json.RootElement.GetProperty("data"); Assert.Equal(1, data.GetProperty("mealPlans").GetArrayLength()); Assert.False(data.TryGetProperty("menus", out _)); var selectedPlan = data.GetProperty("mealPlans").EnumerateArray().Single(); Assert.True(selectedPlan.GetProperty("isSelected").GetBoolean()); Assert.Single(selectedPlan.GetProperty("slots").EnumerateArray()); }
    ...
    private async Task SeedAsync()
    {
        ...
        db.Add(plan); db.Add(secondPlan); db.MealMedia.Add(new() { EntityId = planId, MediaType = MealMediaTypes.MealPlan, ObjectKey = $"meal-plans/{planId:D}/images/plan.png", PublicUrl = "https://cdn.test/plan.png", IsPrimary = true, Status = "ACTIVE", CreatedAt = now, UpdatedAt = now }); await db.SaveChangesAsync();
    }
}
