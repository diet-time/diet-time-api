...
public sealed class CatalogueApiTests : IAsyncLifetime
{
    ...
    [Fact] public async Task Meal_plan_categories_return_real_image_calories_and_price() { if (!enabled) return; using var json = System.Text.Json.JsonDocument.Parse(await client!.GetStringAsync("/api/v1/meal-plan-categories?language=en")); var plans = json.RootElement.GetProperty("data").EnumerateArray().ToArray(); Assert.Equal(2, plans.Length); var classic = plans.Single(x => x.GetProperty("code").GetString() == "CLASSIC"); Assert.Equal("https://cdn.test/plan.png", classic.GetProperty("imageUrl").GetString()); Assert.Equal(420, classic.GetProperty("dailyCaloriesKcal").GetDecimal()); Assert.Equal(349, classic.GetProperty("startingPrice").GetDecimal()); Assert.Equal("QAR", classic.GetProperty("currencyCode").GetString()); Assert.Equal(7, classic.GetProperty("priceDurationDays").GetInt32()); }
    ...
    private async Task SeedAsync()
    {
        ...
        plan.Prices.Add(new() { DurationDays = 7, MealsPerDay = 1, SnacksPerDay = 0, CurrencyCode = "QAR", Amount = 349, EffectiveFrom = now.AddDays(-1), IsActive = true, CreatedAt = now, UpdatedAt = now });
